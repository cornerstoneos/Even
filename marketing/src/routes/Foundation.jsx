...
/* ═══════════════════════════════════════════════════════════════════════════
   ...
   ───────────────────────────────────────────────────────────────────────────
   CAMERA RIG — LOCKED FOR EVERY STAGE OF THE SEQUENCE
   ───────────────────────────────────────────────────────────────────────────
   Stages 02 (framing), 03 (walls) and 04 (finish) MUST reuse AX / AY /
   ORIGIN / VB unchanged so the ground plane sits in exactly the same place
   in every frame of the finished piece. P() is the single projection used
   by all geometry — plan coordinates in, screen coordinates out.
   ...
   COMPOSITION CONTRACT
     · foundation silhouette occupies y 214 → 537 (the lower-middle band)
     · the top ~31% of the frame is deliberately empty — that is the volume
       the framing rises into in stage 02
     · the bottom ~20% is the caption band and stays clear of geometry
     · CAM is the only thing allowed to move between stages, and only as a
       slow dolly along this same axis. Never re-angle the camera.
...
const pts  = list => list.map(([x, y, z = 0]) => P(x, y, z).join(',')).join(' ')
const path = list => 'M' + list.map(([x, y, z = 0]) => P(x, y, z).join(',')).join(' L') + ' Z'

const GOLD    = '#D4AF37'
const GOLD_HI = '#F2D782'
const BG      = '#080808'

/* ── FOUNDATION GEOMETRY ──────────────────────────────────────────────────
   Two poured volumes: a perimeter footing, and the slab that sits on it.
   The 16-unit step between them is what makes the mass read as built
   rather than drawn — you see the shelf, so you believe the thickness.   */
const SLAB   = { x0: -190, x1: 190, y0: -125, y1: 125 }
const FOOT   = { x0: -204, x1: 204, y0: -139, y1: 139 }
const FOOT_Z = [0, 22]
const SLAB_Z = [22, 60]

const rectPlan = (r, z) => [[r.x0, r.y0, z], [r.x1, r.y0, z], [r.x1, r.y1, z], [r.x0, r.y1, z]]

/** Outer outline of an iso box — used for clipping, shadow and glow. */
const silhouette = (r, zb, zt) => [
...
/** One poured volume: top face, two visible side faces, ambient occlusion. */
function Volume({ r, zb, zt, tone }) {
